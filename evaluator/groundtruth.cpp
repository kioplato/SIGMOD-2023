...
knn_of_point(const vector<vector<float>>& points,
		const vector<float>& point, const uint32_t& k)
{
	...
	for (size_t c_point = 0; c_point < points.size(); ++c_point) {
		double distance = euclidean_distance_aprox(point, points[c_point]);
	...
	/*
	 * No, we don't need to reverse the nearest neighbors. The evaluator
	 * program which takes this true knng generated by this script only
	 * cares how many true nn exist in the aprox knng.
	 */
...
sample_true_knng(const vector<vector<float>>& points,
...
int main(int argc, char **argv)
{
	...
	if (n_samples == 0)
		info("generating complete true knng of ", dataset_path, " dataset.");
	...
	// Read the dataset.
	info("reading ", dataset_path, " dataset.");
	vector<vector<float>> points = read_dataset(dataset_path, n_dims);
	info("read ", points.size(), " points from the dataset.");

	// We cannot create a bigger sample than the dataset's cardinality.
